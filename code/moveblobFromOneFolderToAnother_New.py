...
source = "Review_1"
typ = "all"
target = "Hari2_" + str(datetime.datetime.now().date()) + "_" + typ
limit = 1

statusfile = r"D:\Invoice Data Extraction\TAPP 3.0\Tagging_Overall_Summary2020-06-05.xlsx"

df = pd.read_excel(statusfile)
df = df[(df["Folder"] == source)]

if typ == "line_item":
    df = df[(df["line item"] == 0) & (df["header"] == 1) & (df["dup"] == 0)]
elif typ == "all":
    df = df[(df["line item"] == 0) & (df["header"] == 0) & (df["dup"] == 0)]
...
for ind, row in df.iterrows():
    if cnt <= limit:
        startswith = source + "/" + row["Actual File Name"] + "_"
        print(startswith)
        allBlobs = container_client.list_blobs(name_starts_with=startswith)
        for b in allBlobs:
            blobname = b.name
...
localfiles = os.listdir(tempfolder)
